...
link:https://www.apache.org/legal/release-policy.html[ASF policy on software
...
. Before building a release candidate, make sure you have followed the
  ...
  setting up your PGP keys]. In addition to the link:https://pgp.mit.edu/[MIT
  ...
  keyservers] or link:https://keyserver.ubuntu.com[Ubuntu OpenPGP
...
+
NOTE: Don't forget to also update `KUDU_VERSION` and `KUDU_PYTHON_VERSION` in
`examples/flink-replication/.env`.
...
. In the `master` branch of the project git repo, update version-related
  ...
  building the examples. Don't forget to also update `KUDU_VERSION` and
  `KUDU_PYTHON_VERSION` in `examples/flink-replication/.env`.
