...
public class MetricWrappedKuduEnumerator implements SplitEnumerator<KuduSourceSplit,
  ...
  public void handleSourceEvent(int subtaskId, SourceEvent sourceEvent) {
    ...
    if (sourceEvent.getClass().getSimpleName().equals("SplitFinishedEvent")) {
      try {
        Field finishedSplitsField = ReflectionSecurityUtils.getAccessibleField(
            sourceEvent, "finishedSplits");
        List<KuduSourceSplit> finishedSplits = ReflectionSecurityUtils.getFieldValue(
            finishedSplitsField, sourceEvent);

        // Delegate first so the enumerator can schedule additional work immediately.
        // Then, restore invariants to defer actual removal until checkpoint completes.
        ...
        LOG.warn("Failed to intercept SplitFinishedEvent via reflection; " +
  ...
  public void notifyCheckpointComplete(long checkpointId) throws Exception {
    ...
    if (!splitsPendingRemoval.isEmpty()) {
      ...
      for (String splitId : splitsPendingRemoval) {
        ...
        boolean removedFromUnassigned =
            unassigned.removeIf(split -> split.splitId().equals(splitId));

        if (removedFromPending || removedFromUnassigned) {
          removedCount++;
          LOG.debug("Checkpoint {} complete: removed split {} from pending={} unassigned={}",
                    checkpointId, splitId, removedFromPending, removedFromUnassigned);
        }
      }
