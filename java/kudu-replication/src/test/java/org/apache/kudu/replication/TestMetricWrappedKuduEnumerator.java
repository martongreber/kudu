...
public class TestMetricWrappedKuduEnumerator {
  ...
  public void testSplitNotLostWhenFinishedBeforeCheckpointComplete() throws Exception {
    MetricWrappedKuduEnumerator enumerator = createTestEnumerator();
    ...
    String splitId = testSplit.splitId();
    assertTrue(
    ...
    KuduSourceEnumeratorState checkpointAfterComplete = enumerator.snapshotState(2L);
  ...
  public void testMultipleSplitsRetainedBetweenCheckpoints() throws Exception {
    ...
    String split1Id = split1.splitId();
    assertTrue(
    ...
    String split2Id = split2.splitId();
    ...
    KuduSourceEnumeratorState checkpoint3 = enumerator.snapshotState(3L);
    List<KuduSourceSplit> pendingAfterCleanup1 = getPendingSplitsFromState(checkpoint3);

    assertTrue("Both splits should be removed after checkpoint completes",
        pendingAfterCleanup1.stream().noneMatch(s -> s.splitId().equals(split1Id)));
    assertTrue("Both splits should be removed after checkpoint completes",
        pendingAfterCleanup1.stream().noneMatch(s -> s.splitId().equals(split2Id)));
  }
   ...
   * if this test starts PASSING, it means Flink has fixed FLINK-38575 upstream, and we can
  ...
  public void testPlainEnumeratorLosesSplitDueToRaceCondition() throws Exception {
    ...
    KuduSourceEnumeratorState checkpointState = plainEnumerator.snapshotState(1L);
  ...
  private MetricWrappedKuduEnumerator createTestEnumerator() throws Exception {
  ...
  @SuppressWarnings("unchecked")
  private KuduSourceEnumerator createPlainEnumerator() throws Exception {
    KuduTableInfo mockTableInfo = mock(KuduTableInfo.class);
    when(mockTableInfo.getName()).thenReturn("test_table");
  ...
  private KuduSourceSplit createTestSplit(String splitId) throws Exception {
