...
public class TestReplicationMetrics extends ReplicationTestBase {
  ...
  private Long waitForTimestampAdvancement(
          JobID jobId, Long previousTimestamp, long timeoutMs) throws Exception {
    ...
    while (System.currentTimeMillis() < endTime) {
      ...
      if (currentTimestamp != null && currentTimestamp > previousTimestamp) {
        ...
        LOG.info("Timestamp advanced from " + previousTimestamp + " to " + currentTimestamp +
                 " (waited " + waitedMs + "ms)");
    ...
    throw new AssertionError("Timeout waiting for timestamp advancement. " +
            "Previous: " + previousTimestamp + ", Current: " + finalTimestamp +
            ", Waited: " + timeoutMs + "ms");
  }
  ...
  public void testMetricsPresence() throws Exception {
    ...
    Optional<Gauge<Long>> lastEndTimestamp = findMetric(metrics, "lastEndTimestamp");
    Optional<Gauge<Integer>> pendingSplits = findMetric(metrics, "pendingSplits");
    Optional<Gauge<Integer>> unassignedSplits = findMetric(metrics, "unassignedSplits");

    assertTrue("lastEndTimestamp metric should be present", lastEndTimestamp.isPresent());
    assertTrue("pendingSplits metric should be present", pendingSplits.isPresent());
    assertTrue("unassignedSplits metric should be present", unassignedSplits.isPresent());

    // Basic sanity checks
    assertNotNull("lastEndTimestamp should have a value", lastEndTimestamp.get().getValue());
    assertNotNull("pendingSplits should have a value", pendingSplits.get().getValue());
    assertNotNull("unassignedSplits should have a value", unassignedSplits.get().getValue());
