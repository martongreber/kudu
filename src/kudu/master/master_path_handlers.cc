  ...
  for (const auto& master : masters) {
    ...
    for (const auto& host_port : master.registration().http_addresses()) {
      data->master_targets.emplace_back(PrometheusSDData::ServerTarget{
          ...
          "n/a",
          master.registration().https_enabled()});
    }
  }
...
void MasterPathHandlers::HandlePrometheusSD(const Webserver::WebRequest& req,
  ...
  // Check if this master is the leader. For multi-master support, only the leader
  // should return service discovery data. Non-leader masters return an empty array
  // with HTTP 200 status, which allows Prometheus to aggregate results from all
  // masters and append them together.
  CatalogManager::ScopedLeaderSharedLock l(master_->catalog_manager());
  if (!l.catalog_status().ok()) {
  ...
  // TODO(mgreber): Even after the catalog manager becomes ready, consider returning
  // HTTP 503 for an additional grace period of (FLAGS_heartbeat_rpc_timeout_ms +
  // FLAGS_heartbeat_interval_ms) to allow tablet servers time to re-register after
  // a master restart. Without this, Prometheus may briefly receive an incomplete
  // target list.
  if (!l.leader_status().ok()) {
  ...
  // The cluster ID may be transiently empty right after a master restart, before
  // the catalog manager has finished initializing. Return an empty response so
  // Prometheus can use its cached data rather than receiving a broken payload.
  if (data.cluster_id.empty()) {
    WriteEmptyPrometheusSDResponse(output);
    return;
  }
