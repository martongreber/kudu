...
TEST_F(RestCatalogTest, TestApiSpecEndpoint) {
  ...
  VerifyObjectExists(tables_path, "get");
  VerifyObjectExists(tables_path, "post");

  const Value* table_by_id_path;
  ASSERT_OK(reader.ExtractObject(paths, "/tables/{table_id}", &table_by_id_path));
  VerifyObjectExists(table_by_id_path, "get");
  VerifyObjectExists(table_by_id_path, "put");
  VerifyObjectExists(table_by_id_path, "delete");

  const Value* leader_path;
  ASSERT_OK(reader.ExtractObject(paths, "/leader", &leader_path));
  VerifyObjectExists(leader_path, "get");
  ...
  for (const auto& schema_name : expected_schemas) {
    VerifyObjectExists(schemas, schema_name.c_str());
  }
}
