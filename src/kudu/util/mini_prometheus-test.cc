...
class MockKuduServer {
 public:
  ...
  Status Start() {
    // Use a shared string so the SD handler can reference the address after Start().
    auto self_addr = std::make_shared<string>();
    ...
    // Must use RegisterJsonPathHandler so the response has Content-Type:
    // application/json, which is required by the Prometheus HTTP SD spec.
    server_->RegisterJsonPathHandler(
        "/prometheus-sd", "Prometheus SD",
        [self_addr](const Webserver::WebRequest& /*req*/,
    ...
    port_ = addrs[0].port();
    *self_addr = Substitute("127.0.0.1:$0", port_);
    return Status::OK();
  }
...
// GetTargets() with a valid JSON envelope. No Kudu cluster is needed.
...
TEST_F(MiniPrometheusTest, DiscoverAndScrapeTarget) {
  ...
  // Wait up to 60 seconds for the mock target to become healthy.
...
// rather than being discovered via HTTP SD. This exercises the non-SD code path
// in WriteConfig() and confirms MiniPrometheus works for callers that know their
// target addresses upfront.
TEST_F(MiniPrometheusTest, StaticTargetScrape) {
  ...
  // Wait for the statically-configured target to become healthy.
  ASSERT_OK(prom.WaitForActiveTargets(1, MonoDelta::FromSeconds(60)));
...
TEST_F(MiniPrometheusTest, WaitForActiveTargetsTimeout) {
  ...
  Status s = prom.WaitForActiveTargets(1, MonoDelta::FromSeconds(3));
