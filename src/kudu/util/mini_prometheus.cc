...
Status MiniPrometheus::Start() {
  ...
  Status wait = WaitForTcpBind(process_->pid(), &port_, {"127.0.0.1"},
...
string MiniPrometheus::url() const {
  return Substitute("http://127.0.0.1:$0", port_);
}
...
Status MiniPrometheus::WriteConfig() const {
  ...
  if (!options_.master_sd_urls.empty() || !options_.static_targets.empty()) {
    ...
    if (!options_.master_sd_urls.empty()) {
      // Each master SD URL becomes a separate http_sd_configs entry within a
      // single scrape job so Prometheus aggregates all the target lists together.
      // The leader returns the full target list; followers return [] (HTTP 200),
      // and Prometheus deduplicates the merged result.
...
Status MiniPrometheus::FetchJson(const string& path, rapidjson::Document* doc) {
  EasyCurl c;
  c.set_timeout(MonoDelta::FromSeconds(10));
  faststring buf;
...
Status MiniPrometheus::Query(const string& promql, rapidjson::Document* doc) {
  ...
  c.set_timeout(MonoDelta::FromSeconds(10));
  faststring buf;
  const string url_str = Substitute("$0/api/v1/query?query=$1", url(), promql);
