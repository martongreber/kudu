...
// 'deadline' passes (if it is initialized). If any read yields an error, it is
// returned. On a clean drain, 'out' contains the bytes read for each fd, in the
// same order as was in 'fds'. If the deadline passes first, '*timed_out' is set
// and 'out' holds whatever was read before then. '*timed_out' is always
// written; the caller may pass a throwaway bool when no deadline is used.
Status ReadFdsFully(const string& progname,
  ...
  *timed_out = false;
  ...
  // Arm a deadline timer that breaks the loop, if a deadline was requested.
  unique_ptr<DeadlineHelper> deadline_helper;
  if (deadline.Initialized()) {
    deadline_helper.reset(new DeadlineHelper(
        &loop, (deadline - MonoTime::Now()).ToSeconds()));
  }
...
Status Subprocess::WaitAndCollect(const MonoTime& deadline,
  ...
  *timed_out = false;
  ...
  vector<string> outv;
  RETURN_NOT_OK(ReadFdsFully(program_, fds, deadline, &outv, timed_out));
  ...
  if (*timed_out) {
    ...
    if (!s.ok()) {
      LOG(WARNING) << "failed to kill timed-out child " << program_ << ": "
                   << s.ToString();
    }
  }
...
Status Subprocess::Call(const vector<string>& argv,
  ...
  vector<string> outv;
  bool timed_out;
  // No deadline: drain to EOF, matching the historical behavior of Call().
  RETURN_NOT_OK(ReadFdsFully(argv[0], fds, MonoTime(), &outv, &timed_out));
