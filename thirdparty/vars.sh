...
if [ -z "$KUDU_CDH_GBN" ]; then
  ...
  if [ -z "$KUDU_CDH_GBN" ]; then
    echo "Error: failed to fetch CDH global build number(GBN)"
    exit 1
  fi
  export KUDU_CDH_GBN=$KUDU_CDH_GBN
fi

BUILD_URL_PREFIX="${CAULDRON_URL_PREFIX}/${KUDU_CDH_GBN}/impala-minicluster-tarballs"
